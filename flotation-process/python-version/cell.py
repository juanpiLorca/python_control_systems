...
class Cell(): 
    ...
    args: 
        >>> cell_id: ID of each cell in the Line
        >>> i: ore index
        >>> num_classes: number of bubble classes
        >>> sampling_period: sampling period to compute ODEs solutions
        >>> u1Var: increment/decrease floating number for valve openning FIC
        >>> u2Var: increment/decrease floating number for air flux.
    """

    def __init__(self, cell_id: int, i: int, num_classes: int, params: dict, 
                 Qfeed: float):
        ...
        # Model parameters: 
        self.params = params
        epsis = np.array(self.params["epsis"])
        ...
        # Activate keys as actuators: 
        keyboard.hook(self.actuators)
    ...
    def odefun(self, x, t): 
        ...
        if not not_solve: 
            ...
            Dh0Dt = (1 / self.params["Acell"]) * (self.Qfeed - Qpulp_out)
            DghDt = ((1 + gh_tot) / (self.params["Acell"] * self.hp)) * (self.Qair_k - self.params["Acell"]
                        * vg_k_pulp_out * (gh / (1 + gh_tot)) - (self.Qfeed - Qpulp_out) * gh)
    ...
    def model_equations(self, t, x): 
        ...
        # Masses: Mass Balance 
        mfeed = np.array(self.params["Cfeed"]) * self.Qfeed
        mtail = Ctail * Qtail
    ...
    def simulate(self): 
        ...
        m = x[-1,:self.i]
        h0 = x[-1,self.i]
        gh = x[-1,self.i+1:self.i+self.K+1]
        dbf = x[-1,self.i+self.K+1]
...
#     cell = Cell(cell_id=cell_id, i=i, num_classes=K, params=params, 
#                 Qfeed=Qfeed)

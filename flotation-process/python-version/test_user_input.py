...
def odefun(t,x):
    ...
    Qconc = s[20]
    Qtail = s[21]
    Qtail_lista.append(Qtail)

    if len(Qtail_lista) > 1:
        #print(Qtail_lista)
        #print(len(Qtail_lista))
        Qtailt1 = Qtail_lista[len(Qtail_lista)-1]
        Qtailt0 = Qtail_lista[(len(Qtail_lista)-2)]
        rev = (Qtailt1 - Qtailt0) / Qtailt1
        rev_abs = abs(rev)
        #print(rev_abs)
        if rev_abs < 0.0000000000001:#0.00000000000001:
            u1_new = float(input("Entrega un valor de apertura de válvula"))
            u2_new = float(input("Entrega un valor de flujo de aire"))
            u = [u1_new, u2_new]
            U.append(u)
            tU.append(t)
    else:
...
def model_equations(x,odefun=1,t=None):

    global u, counter

    if not odefun and counter<len(tU):
        if t>=tU[counter]:
            counter+=1
            u = U[counter]
            print(u)
    ...
    initial_guess = [0.003, 0.03]#[1e-5,1e-5]
    ...
    return np.concatenate(([gh_tot], [hp], Qair_k, vg_k_pulp_out, mfeed, mtail, mTF, mENT, [Qconc] , [Qtail], [alpha], [not_solve]))
...
#     model_vars.append(model_equations(solution.y[:,k],odefun=0,t=solution.t[k]))
...
ax[0,0].plot(TIME,MT1) # estado 0
ax[0,0].plot(TIME,MT2) # estado 1
ax[0,0].set_title('Masa total de cada mineral')

ax[0,1].plot(TIME,MTF)
ax[0,1].plot(TIME,MENT)
ax[0,1].set_title('Mtf y Ment de interes economico')

ax[1,0].plot(TIME,HP)
ax[1,0].set_title('Altura de la pulpa en la celda')

ax[1,1].plot(TIME,ALPHA)
ax[1,1].set_title('Recuperacion de aire')

ax[2,0].plot(TIME,QTAIL)
ax[2,0].set_title('Flujo de cola')

ax[2,1].plot(TIME,QCONC)
